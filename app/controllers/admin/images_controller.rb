...
class Admin::ImagesController < Admin::AdminController
  ...
  # The options are needed because the app will use them to ensure at least
  # rudimentary cropping (in the absence of admin created crops) and the URLs
  # need to match exactly. For images created by the create_images method, they
  # do not represent the cropping actually done.
  IMG_OPTIONS = ',crop=center,grow=1'
  ...
  def self.create_images(cropped, group_name, cache_dir, cache_path, suffix)
    ...
    if group
      ...
      # store that. The dimensions are already integers, via to_i.
      cropped_filename = "/thumbnail.width=#{w},height=#{h}#{IMG_OPTIONS}#{suffix}"
